...
export const getAll = async (filter) => {
  const arrayMetaTranslated = filter.meta.split(',').map((item) => {
    if (Number(item) === 1) return undefined;
    if (Number(item) === 2) return 'pending';
    if (Number(item) === 3) return 'accepted';
    return null;
  });
  if (Number(filter.meta.split(',').length) === 0) arrayMetaTranslated.push(filter.meta);
  const optionType = {
...
export const updateBalance = async (user, payload) => {
  ...
  if (payload.status) {
    currentUser.balance =
      parseInt(currentUser.balance, 10) + parseInt(payload.amount, 10);
    currentUser.save();
  } else {
    currentUser.balance =
      parseInt(currentUser.balance, 10) - parseInt(payload.amount, 10);
...
export const choiceTheAdminSupport = async () => {
  ...
  await Promise.all(
    data.map(async (item) => {
      ...
      if (
        totalUser.filter((e) => moment().isSame(e.created_at, 'day')).length <
        min
      ) {
        min = totalUser.filter((e) =>
          moment().isSame(e.created_at, 'day')
        ).length;
        res = item;
      }
    })
...
export const userRegisterToken = async (
  ...
  const optionsRegis = {};
  if (token) optionsRegis['tokenPushNotification'] = token;
  if (platform) optionsRegis['platform'] = platform;
  if (deviceToken) optionsRegis['deviceToken'] = deviceToken;
